...
// const getAllToDoListsByUserId=async(req,res)=>{
...
exports.createToDoList=async(req,res)=>{
    const { userId } = req.params;
    const { data } = req.body;
    // const userTasks = await ToDoListModel.filter((each) => each.userId === userId);
    const userTasks = await ToDoListModel.find({ userId: userId });
    if (!userTasks) {
        return res.status(404).json({
            success: false,
    ...
    await newTask.save();
    // const updatedList = await ToDoListModel.filter((each) => each.userId === userId);
